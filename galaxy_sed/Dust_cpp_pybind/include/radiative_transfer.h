...
namespace radiative_transfer
{
    ...
    private:
        ...
        GClumpVal(const val &tau_clump, const val &g_val, const val &omega) noexcept
        {
            ...
            for (auto i = std::size_t(0); i < N_LAMBDA; ++i)
            {
                ...
                g_clump_val[i] = g - C * (1 - (1 + std::exp(-B / A)) / (1 + std::exp((tau_clump[i] - B) / A)));
        ...
        [[nodiscard]] inline val2 PhaseFunctionVal(double g) const noexcept
        {
            ...
            for (auto i = std::size_t(0); i < N_ANGLE; ++i)
            { //
                for (auto j = std::size_t(0); j < N_ANGLE; ++j)
                { // 入射角
                                                      ...
                                                      const val &phase_function_val) const noexcept
        {
            ...
            {
                scattering_emissivity_val += 0.5 * dMU * (I_val2[i] * phase_function_val[i] + I_val2[i - 1] * phase_function_val[i - 1]);
            }
        ...
        IntensityOneStep(double d_tau_mu, double I0, double S0, double S1) const noexcept
        {
            ...
            return d_tau_mu > 1e-3 ? I0 * exp_d_tau_mu + (inv_d_tau_mu - exp_d_tau_mu * inv_d_tau_mu - exp_d_tau_mu) * S0 + (1 - inv_d_tau_mu + exp_d_tau_mu * inv_d_tau_mu) * S1
                                   // small tau case
                                   : I0 * exp_d_tau_mu + (0.5 * d_tau_mu - d_tau_mu * d_tau_mu / 3) * S0 + (0.5 * d_tau_mu - d_tau_mu * d_tau_mu / 6) * S1;
        }
                                                          ...
                                                          const val2 &phi_val2) const noexcept
        {
            ...
            for (auto n = std::size_t(0); n < N_Ng_MAX; ++n)
            {
                ...
                if (IsConvergence(old_S_val2, S_val2))
                    return I_val2;
    ...
    public:
        ...
        inline RadiativeTransfer(double D, double dz, val f_young_val, const val &k_val,
                                 const val &omega_val, const val &g_val, double n0_cnm) noexcept : D_(D),
        ...
        inline void WriteMGAParameter(const FreeParameter &free_params)
        {
            ...
            for (auto li = std::size_t(0); li < N_LAMBDA; ++li)
            {
                ...
                if (std::isnan(extinction_val[li]))
                    ofs << 0.0 << std::endl;
                else
                    ofs << extinction_val[li] << std::endl;
            }
            ofs.close();
        }
