...
namespace my_util::dust_util
{
    ...
    private:
        ...
        {
            if (dust_species_ == "Sil")
                return SILICATE_SUBLIMATION_TEMPERATURE;
        ...
        inline void ReadCoolingTimeFile()
        {
            try {
                ...
                throw std::runtime_error("Memory allocation failed in ReadCoolingTimeFile: " + std::string(e.what()) +
                                        " | n_a_ = " + std::to_string(n_a_));
            }
            for (auto di = std::size_t(0); di < n_a_; ++di)
            {
                ...
                for (auto T = std::size_t(0); T < N_TEMPERATURE; ++T)
                {
                    ...
                    ifs >> buff >> t_cool_val2_[di][T] >> t_int_cool_val2_[di][N_TEMPERATURE - T] // 昇順になるように逆から挿入する
                        >> epsilon_val2_[di][T];
                }
            }
        }
        ...
        [[nodiscard]] inline std::size_t SearchTemperatureIterationNumber(double T) const noexcept
        {
            ...
            if (iter_T == std::end(T_val_)) {
                std::cerr << "Warning: SearchTemperatureIterationNumber exceeded bounds, resetting to last index." << std::endl;
                return N_TEMPERATURE - 1;
            }
                ...
                double &E_internal_now) noexcept
        {
            ...
            if (heat_after_iter == std::end(E_internal_val))
                return N_TEMPERATURE - 1;
        ...
        Cooling(std::size_t i_T, double t_delta, const val &t_integral_cool_val) noexcept
        {
            ...
            if (std::isnan(t_integral_cool_val[N_TEMPERATURE - i_T])) {
                std::cerr << "ERROR: t_integral_cool_val[" << (N_TEMPERATURE - i_T) << "] is NaN." << std::endl;
            }

            auto t_after = t_integral_cool_val[N_TEMPERATURE - i_T] + t_delta;
            if (std::isnan(t_after)) {
                std::cerr << "ERROR: t_after is NaN. t_integral_cool_val[" << (N_TEMPERATURE - i_T) << "] = "
                        << t_integral_cool_val[N_TEMPERATURE - i_T]
                        << ", t_delta = " << t_delta
                        << ", i_T = " << i_T << std::endl;
            }
            if (t_after < 0) {
                std::cerr << "Warning: t_after is negative, setting to minimum value." << std::endl;
                t_after = 0;
            }
            if (std::isnan(t_after) || std::isinf(t_after)) {
                std::cerr << "Warning: t_after is NaN or Inf. Setting to minimum value." << std::endl;
                t_after = 0; // TODO: 仮修正です
            }

            // std::cout << "t_after = " << t_after << ", i_T = " << i_T << std::endl;

            // lower_bound は t_after を超える初めのイテレータを返すがその前がほしいので -1 する
            auto iter_after = std::lower_bound(std::begin(t_integral_cool_val), std::end(t_integral_cool_val), t_after);
            if (iter_after == std::begin(t_integral_cool_val)) {
                std::cerr << "Warning: Cooling index underflow, resetting to first index." << std::endl;
                return N_TEMPERATURE - 1;
            }
            if (iter_after != std::begin(t_integral_cool_val)) {
                --iter_after;  // ✅ ここを変更
                                 ...
                                 double &E_inter_now) noexcept
        {
            ...
            {
                if (i_T_cool_after >= N_TEMPERATURE) {
                    std::cerr << "Warning: i_T_cool_after exceeds N_TEMPERATURE, resetting to max." << std::endl;
                    i_T_cool_after = N_TEMPERATURE - 1;
                }
            ...
            if (t_delta - t_elapsed < 0)
                std::cout << "t_delta - t_elapsed is minus!! t_delta = " << t_delta << ", t_elapsed = " << t_elapsed
                          ...
                          const val &epsilon_val) const noexcept
        {
            ...
            for (auto i = std::size_t(0); i < n_hit; ++i)
            {
                ...
                if (i_T >= N_TEMPERATURE) {
                    std::cerr << "Warning: i_T exceeds N_TEMPERATURE in CoolingAndHeating, resetting to max." << std::endl;
                    i_T = N_TEMPERATURE - 1;
                }
                // 昇華温度に達していた場合を判断する
                ...
                if (std::isnan(t_hit_val[i])) {
                    throw std::runtime_error("ERROR: t_hit_val[" + std::to_string(i) + "] is NaN.");
                }
                if (std::isnan(t_before)) {
                    throw std::runtime_error("ERROR: t_before is NaN.");
                }
    ...
    public:
        ...
        inline Dust(std::string dust_species, val a_cm_val) : dust_species_(std::move(dust_species)),
                                                              n_a_(NumberOfDustRadius()),
                                                              T_sublimation_(SublimationTemperature()),
                                                            //   a_cm_val_(std::move(a_cm_val)), メモリエラー可能性ポイント
                                                              a_cm_val_(a_cm_val),
        ...
        {
            // ✅ ここに異常チェックを追加
            if (n_a_ <= 0 || n_a_ > 1e6) { // 100万以上なら異常とみなす
                throw std::runtime_error("Invalid n_a_ value: " + std::to_string(n_a_));
            }

            // std::cout << "Dust Constructor: n_a_ = " << n_a_ << ", a_cm_val_.size() = " << a_cm_val_.size() << std::endl;

            if (a_cm_val_.size() != n_a_) {
                throw std::runtime_error("Mismatch in a_cm_val_ size and n_a_");
            }
        ...
        DpDt(const val &lambda_cm_val, const val &E_photon_val, const val &u_cgs_val) const noexcept
        {
            ...
            for (auto di = std::size_t(0); di < n_a_; ++di)
            {
                for (auto li = std::size_t(1); li < N_LAMBDA; ++li)
                {
                    ...
                    dpdt_val2[di][li] = PI * a_cm_val_[di] * a_cm_val_[di] * Qabs_val2_[li][di] * u_cgs_val[li] * lambda_cm_val[li] *
                                        lambda_cm_val[li] * lambda_cm_val[li] / (h_P * h_P * cl) * dE;
                }
            }
            return dpdt_val2;
        }
        ...
        DustRadiation(std::size_t ai, const val2 &T_hist_val2, const val2 &BB_val2) const noexcept
        {
            ...
            std::cout << "L_val finished" << std::endl;
            ...
            std::cout << "for li finished" << std::endl;
            return L_val;
        }
                                                ...
                                                const FreeParameter &free_params) const noexcept
        {
            ...
            ofs << "Wavelength[cm] ";
            std::cout << "ofs finished" << std::endl;
            for (auto i = std::size_t(0); i < n_a_; ++i)
            {
                ofs << a_cm_val_[i] << " ";
            }
            ofs << std::endl;
            std::cout << "ofs2 finished" << std::endl;
            for (auto li = std::size_t(0); li < N_LAMBDA; ++li)
            {
            ...
            std::cout << "LuminosityOfOneLambdaPerRadius finished" << std::endl;
        }
    };
     ...
     */
    inline std::vector<Dust> MakeDustClassVector(const std::vector<std::string> &dust_species_vec, const val &a_cm_val)
    {
        // ダストクラスのベクトルを格納するための空のベクトルを作成
        auto dust_vec = std::vector<my_util::dust_util::Dust>();

        // 各ダスト種に対してループ
        for (const auto &dust_species : dust_species_vec) {
            // PAHionまたはPAHneuの場合、特定のダスト半径の数を設定
            std::size_t n_a = (dust_species == "PAHion" || dust_species == "PAHneu") ? N_DUST_RADIUS_PAH : N_DUST_RADIUS;

            // a_cm_valのサイズが期待される数より小さい場合、エラーをスロー
            if (a_cm_val.size() < n_a) {
                throw std::runtime_error("a_cm_val size is smaller than expected: " + std::to_string(a_cm_val.size()) + " < " + std::to_string(n_a));
            }

            // a_cm_valからn_aに合わせて部分配列を切り取る
            val a_cm_val_subset = a_cm_val[std::slice(0, n_a, 1)]; // ✅ n_a に合わせて切り取る

            // ダストの種類とその半径のサイズを出力
            std::cout << "Creating Dust: " << dust_species << ", n_a = " << n_a << ", a_cm_val_subset.size() = " << a_cm_val_subset.size() << std::endl;

            // ダストクラスのインスタンスを作成し、ベクトルに追加
            dust_vec.emplace_back(dust_species, a_cm_val_subset);
        }
    ...
    [[nodiscard]] inline double PAHneuFraction(double a_cm) noexcept
    {
        ...
        return (1.0 / (1.0 + 3.5e6 * 0.460 * std::pow(radius_um, 3) * 1.14 * std::pow(100, 0.5) / 0.045) + 1.0 / (1.0 + 3.5e6 * 0.460 * std::pow(radius_um, 3) * 1.14 * std::pow(6000, 0.5) / 0.04)) * 0.5;
    }
