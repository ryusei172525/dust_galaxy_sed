...
inline double IMF(double m) noexcept {
    switch (IMF_TYPE) {
...
inline double IMFNormalization(double m_min, double m_max) noexcept {
    const auto dm = 0.01;
    ...
    return dm / 3.0 * (0.1 * IMF(m_min) + 4.0 * s1 + 2.0 * s2 + 100 * IMF(m_max));
}


inline val NormalizedIMF() noexcept {
    auto normed_IMF = val(N_M_STAR);

    const auto IMF_0 = IMFNormalization(0.1, 100);

    for (auto i = std::size_t(0); i < N_M_STAR; ++i) {
        const auto m = 1.0 + 0.01 * static_cast<double>(i);
        normed_IMF[i] = IMF(m) / IMF_0;
    }
    return normed_IMF;
}
