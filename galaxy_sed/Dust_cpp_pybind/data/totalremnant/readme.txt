totalremnantA,B,Cディレクトリに入っているデータは
...
1. interpolate_rem_**.datを用いてそれぞれのデータをmetallicity方向に線型補間
　 補間したデータはnewyield/**rem_interpolate/に入っている)
   なお、このディレクトリに入っているデータは左から星質量(Msun),
   remnant mass (Msun)となっている。

2. newyield/**rem_interpolate/に入っているデータを用いて
   星質量方向に0.01Msun刻みで線型補間。ただし、6-13MsunはそれぞれAGB,SNの
   データを用いて線型補間する。
   metallicityに関しては、AGBが0.005Zsunからしかないので、SNもこれに合わせる。
   使用したプログラムはinterpolate_rem_smass.c
